...
<template>
  <section class="p-6 space-y-6">
    <div>
      ...
      <p v-if="subtitle" class="text-[13px] text-surface-500 mt-0.5">{{ subtitle }}</p>
    </div>

    <div
      class="card p-12 flex flex-col items-center justify-center text-center gap-3 text-surface-500"
    >
      <Construction :size="32" :stroke-width="1.75" class="text-surface-400" />
      <div>
        <div class="text-[14px] font-medium text-surface-700">Écran en cours d'implémentation</div>
        <div class="text-[12px] mt-1">
          Design system et shell en place. Cet écran sera implémenté dans une PR dédiée.
        </div>
      </div>
    </div>
  </section>
